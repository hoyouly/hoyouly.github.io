...
### Hierarchy Viewer
* 可以通过他查看布局的嵌套情况
* 使用 [View Server](https://github.com/romainguy/ViewServer) 可以在普通的手机上使用 Hierarchy Viewer

1. 通过开发者选项中的GPU过度可视化工具，查看界面渲染情况，
3. 可以使用 Layout Inspector 辅助分析。捕捉当前页面快照。生成一个.li文件，通过AndroidStudio查看页面视图层次结构
2. Profile GPU Rendering     也是在开发者选项中打开
    打开后的界面如下
...
1. 通过开发者选项中的GPU过度可视化工具，查看界面渲染情况，
可以使用 Layout Inspector 辅助分析。捕捉当前页面快照。  生成一个.li文件，通过  用来查看页面视图层次结构
2. Profile GPU Rendering     也是在开发者选项中打开
    打开后的界面如下
    ![Alt text](https://github.com/hoyouly/BlogResource/raw/master/imges/carsetting_1.png)
    各个颜色的意思

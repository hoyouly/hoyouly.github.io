...
* 如果不是 merge 标签的话，调用 createViewFromTag() 创建 一个View，然后调用 rInflateChildren(),这个内部还是调用的 rInflate() 进行遍历的
...
如果是 focus/tag/include/merge 这四种标签，单独执行相应的方法去解析。否则 通过 createViewFromTag() 创建 一个View， 然后添加到父布局中 。
...
## inflate() 参数意义
* resource  我们知道，就是加载布局的资源id，这个没啥好解释的
* root   该布局外部嵌套一层父布局
...
* root 不为 null, attachToRoot 设为 true, 将layout布局添加到root布局中,不需要手动调用 addView()
* root 不为 null, attachToRoot 设为 false, 不将layout布局添加到root布局，若要添加则需要手动 addView()
* root 不为 null，不设置 attachToRoot，  attachToRoot 的默认值是 true.

## 例子说明
定义一个Activity,加载一个 activity_layoutinfalter.xml 布局，定义一个子布局 child_layout.xml，里面就一个Button,
...
发现并不是我们想的那样，Button 尽管我们设置了 宽高都是200dp，可是，宽确实match_parent，高 是warp_content,这不和我们的逻辑啊。
...
符合我们的要求，也验证了上面的结论 该 view 被添加到父 view 当中时，这些 layout 属性会自动生效。因为 layout_width 和 layout_width 并不是设置View的大小，而是设置View在布局中的大小。也就是说View必须存在于一个布局中，layout 属性才会生效，
